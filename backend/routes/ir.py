...
from utils import api_success, api_error, api_from_result, parse_int, SyncTask

bp = Blueprint('ir', __name__, url_prefix='/api')


def _ir_db():
    """Lazy-init the IR payload database."""
...
def ir_record():
    ...
    ir = get_module('ir')
    return api_from_result(ir.record_signal(duration=duration))
...
def ir_transmit():
    ...
    return api_from_result(result, error_code=400)
...
def ir_library_send(device_id):
    ...
    if not pulses and not spaces:
        ...
        from modules.ir.protocols import encode as proto_encode
...
def ir_sync_start():
    def _run_sync():
        ...
        db = _ir_db()
        syncer = IRDBSync(db)
        return syncer.sync(progress_callback=_sync.callback)
    return _sync.start(_run_sync)
